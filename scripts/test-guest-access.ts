...
console.log(
  `ANONYMOUS_MODELS: ${process.env.ANONYMOUS_MODELS || 'all (default)'}`,
);
console.log(
  `ANONYMOUS_CREDITS: ${process.env.ANONYMOUS_CREDITS || '10000 (default)'}`,
);
console.log(`ANONYMOUS_RPM: ${process.env.ANONYMOUS_RPM || '10000 (default)'}`);
console.log(
  `ANONYMOUS_RPMONTH: ${process.env.ANONYMOUS_RPMONTH || '10000 (default)'}`,
);
...
console.log(
  `Rate Limit (RPM): ${ANONYMOUS_LIMITS.RATE_LIMIT.REQUESTS_PER_MINUTE}`,
);
console.log(
  `Rate Limit (Monthly): ${ANONYMOUS_LIMITS.RATE_LIMIT.REQUESTS_PER_MONTH}`,
);
console.log(`Session Duration: ${ANONYMOUS_LIMITS.SESSION_DURATION}`);
console.log(
  `Available Tools: ${ANONYMOUS_LIMITS.AVAILABLE_TOOLS.length} tools`,
);
...
console.log(
  `Models available for guests: ${ANONYMOUS_LIMITS.AVAILABLE_MODELS.length}`,
);
...
if (envModels.toLowerCase() !== 'all') {
  const requestedModels = envModels
    .split(',')
    .map((s) => s.trim())
    .filter((s) => s.length > 0);
...
if (ANONYMOUS_LIMITS.AVAILABLE_MODELS.length === 0) {
  console.log(
    '❌ No models available for guests! Check ANONYMOUS_MODELS env var.',
  );
  ...
  console.log(
    `⚙️ ${ANONYMOUS_LIMITS.AVAILABLE_MODELS.length}/${chatModels.length} models available for guests`,
  );
}
